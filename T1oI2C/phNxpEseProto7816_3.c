...
bool_t phNxpEseProto7816_Transceive(phNxpEse_data *pCmd, phNxpEse_data *pRsp)
{
    ...
    LOG_D("\nTransceive data ptr 0x%p len:%ld ", pCmd->p_data, pCmd->len);
    phNxpEseProto7816_SetFirstIframeContxt();
    status = TransceiveProcess();
    if(FALSE == status)
    {
        /* ESE hard reset to be done */
        LOG_E("\n%s Transceive failed, hard reset to proceed ",__FUNCTION__);
    }
    else if(pRsp->len > reqDataLen )
    {
        LOG_W("\nNeed '%d' bytes. Got '%d' to copy.", pRsp->len, reqDataLen);
        pRsp->len = 0;
        status = FALSE;
    }
    else
